defmodule Writer do
  ...
  @doc """
  Assigns a department to an employee and writes the updated employee to the JSON file.

  ## Parameters
  - `employee_id`: Integer, the ID of the employee
  - `department`: %Department{}, the department to assign
  - `filename`: String, the name of the JSON file to write to (optional, default: "employees.json")
  ...
  """
  @spec assign_department_employee(integer(), Department.t(), String.t()) :: :ok | {:error, term()}
  def assign_department_employee(employee_id, department, filename \\ "employees.json") do
    case Reader.read_employee_by_id(employee_id, filename) do
      {:ok, employee} ->
        updated_employee = Map.put(employee, :department, department)
        employees = Reader.read_all_employees(filename)
        updated_employees = [updated_employee | Enum.reject(employees, &(&1.id == employee_id))]
        File.write(filename, Jason.encode!(updated_employees))
