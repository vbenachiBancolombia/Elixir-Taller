defmodule Reader do
  ...
  @doc """
  Finds a department by its ID from the JSON file.

  ## Parameters
  - `id`: Integer, the ID of the department to find
  - `filename`: String, the name of the JSON file to read from (optional, default: "employees.json")

  ## Returns
  - `{:ok, Department.t()}` if the department is found
  - `{:error, :not_found}` if the department is not found
  """
  @spec find_department_by_id(integer(), String.t()) :: {:ok, Department.t()} | {:error, :not_found}
  def find_department_by_id(id, filename \\ "employees.json") do
    employees = read_all_employees(filename)

    case Enum.find(employees, &(&1.department && &1.department.id == id)) do
      nil -> {:error, :not_found}
      employee -> {:ok, employee.department}
    end
  end
